...
###Find taxonomy files
file.names <- dir(InDir, pattern = "*.summary", full.names = TRUE, ignore.case = TRUE, recursive = TRUE)
print(file.names)
print("length")
...
##Extract Barcode Name
  ...
  BARCODE <- gsub(pattern = "\\.tsv.summary$", "", NAME)
  BARCODE2 <- gsub(pattern = paste(SampleID, ".", sep=""), "", BARCODE)
#  BARCODE2 <- paste0("BARCODE_", BARCODE1)
  print(BARCODE2)  
  ##Extract read number from OTU-name
  out <- file$taxid
  out2 <- cbind(file[c(1:5)])
  head(out2)  
  #Rename column names
  colnames(out2) <- c("Taxid","Taxonomic.Name", "Taxonomic.Level" ,"Lineage","Total.Reads")
